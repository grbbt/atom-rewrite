...
class MusicPlayer:
    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            video = pafy.new(source.web_url)
            if video.length is None:

                embed = discord.Embed(title=f"Now Streaming: {video.title}", url=source.web_url,
                                      description=f"by {video.author}", color=0xe1ff00)
                embed.set_thumbnail(url=video.bigthumbhd)
                embed.add_field(name="Duration", value="LIVE", inline=True)
                embed.add_field(name="Views", value="LIVE", inline=True)
                embed.set_footer(text=f"Requested by {source.requester}, This is a livestream, audio may differ in quality")
                livestream = True

            else:
                embed = discord.Embed(title=f"Now Playing: {video.title}", url=source.web_url,
                                      description=f"by {video.author}", color=0xf50000)
                embed.set_thumbnail(url=video.bigthumbhd)
            ...
            self.current = source
            if livestream is True:
                self.np = await self._channel.send("The Livestream will occasinally cut out as LIVE is very much still WIP")
                self._guild.voice_client.play(source, after=lambda _: self.bot.loop.call_soon_threadsafe(self.next.set))
            else:
                audio = video.getbestaudio()
                besturl = discord.FFmpegPCMAudio(audio.url, **ffmpegopts)
                self._guild.voice_client.play(discord.PCMVolumeTransformer(besturl), after=lambda error: self.next.set)
                self._guild.voice_client.source.volume = 0.5
...
class Music(commands.Cog):
    """Music related commands. Still very WIP"""
    ...
    async def play_(self, ctx, *, search: str):
        """Request a song and add it to the queue. Input either link or video query"""
    ...
    @commands.command(name='remove', aliases=['rm', 'rem'], description="removes specified song from queue", hidden=True)
    ...
    @commands.command(name='np', aliases=['song', 'current', 'currentsong', 'playing'],
                      description="shows the current playing song", hidden=True)
